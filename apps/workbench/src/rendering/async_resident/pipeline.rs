...
const RESET_SHADER: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/async_resident.reset.dxil"));
const CULL_SHADER: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/async_resident.cull.dxil"));
const VERTEX_SHADER: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/async_resident.vs.dxil"));
const PIXEL_SHADER: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/async_resident.ps.dxil"));
pub const ASYNC_CONSTANT_COUNT: u32 = 48;

pub struct AsyncResidentPipeline {
...
unsafe fn create_compute_root(device: &ID3D12Device) -> Result<ID3D12RootSignature> {
    let range = srv_range();
    let parameters = [
        root_constants(0, ASYNC_CONSTANT_COUNT),
...
unsafe fn create_graphics_root(device: &ID3D12Device) -> Result<ID3D12RootSignature> {
    ...
    let parameters = [
        root_constants(0, ASYNC_CONSTANT_COUNT),
        descriptor_table(&range),
        root_descriptor(D3D12_ROOT_PARAMETER_TYPE_SRV, ASYNC_CACHE_CAPACITY as u32),
    ];
    unsafe {
        serialize_root(
            device,
...
fn srv_range() -> D3D12_DESCRIPTOR_RANGE {
...
unsafe fn serialize_root(
    ...
    .context("async resident root signature serialization failed")?;
    let blob: ID3DBlob = blob.context("async resident root signature returned no blob")?;
    let bytes = unsafe {
        std::slice::from_raw_parts(blob.GetBufferPointer().cast::<u8>(), blob.GetBufferSize())
    };
    unsafe { device.CreateRootSignature(0, bytes) }
        .context("async resident root signature creation failed")
}
...
unsafe fn create_compute_pipeline(
    ...
    result.context("async resident compute pipeline creation failed")
}
...
unsafe fn create_graphics_pipeline(
    ...
    unsafe { ManuallyDrop::drop(&mut desc.pRootSignature) };
    result.context("async resident graphics pipeline creation failed")
}
...
unsafe fn create_command_signature(device: &ID3D12Device) -> Result<ID3D12CommandSignature> {
    ...
    unsafe { device.CreateCommandSignature(&desc, None, &mut signature) }
        .context("async resident command signature creation failed")?;
    signature.context("async resident command signature returned no signature")
}
