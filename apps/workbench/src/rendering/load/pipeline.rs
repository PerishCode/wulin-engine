...
const RESET_SHADER: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/region_load.reset.dxil"));
const CULL_SHADER: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/region_load.cull.dxil"));
const VERTEX_SHADER: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/region_load.vs.dxil"));
const PIXEL_SHADER: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/region_load.ps.dxil"));
pub const LOAD_CONSTANT_COUNT: u32 = 21;
...
unsafe fn create_compute_root(device: &ID3D12Device) -> Result<ID3D12RootSignature> {
    let parameters = [
        root_constants(0, LOAD_CONSTANT_COUNT),
...
unsafe fn serialize_root(
    ...
    .context("load root signature serialization failed")?;
    let blob: ID3DBlob = blob.context("load root signature serialization returned no blob")?;
    let bytes = unsafe {
        std::slice::from_raw_parts(blob.GetBufferPointer().cast::<u8>(), blob.GetBufferSize())
    };
    unsafe { device.CreateRootSignature(0, bytes) }.context("load root signature creation failed")
}
...
unsafe fn create_compute_pipeline(
    ...
    result.context("load compute pipeline creation failed")
}
...
unsafe fn create_graphics_pipeline(
    ...
    unsafe { ManuallyDrop::drop(&mut desc.pRootSignature) };
    result.context("load graphics pipeline creation failed")
}
...
unsafe fn create_command_signature(device: &ID3D12Device) -> Result<ID3D12CommandSignature> {
    ...
    unsafe { device.CreateCommandSignature(&desc, None, &mut signature) }
        .context("load command signature creation failed")?;
    signature.context("load command signature creation returned no signature")
}
