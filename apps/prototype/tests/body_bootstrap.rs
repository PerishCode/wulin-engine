...
fn motion_touches_terrain() {
    ...
    let motion = body::initial_motion(position, terrain).unwrap();
...
fn invalid_heights_fail() {
    ...
    assert!(body::initial_motion(position, invalid).is_err());
    ...
    assert!(body::initial_motion(position, overflow).is_err());
}
