...
- Keep `sidecar.toml` target-free until a real long-lived client, server, asset daemon,
  or inspection process exists. Do not model one-shot experiments as lifecycle targets.
...
projects. Sidecar contributes project identity and validation now, but no runtime process
ownership until the topology is real.

## Evidence

- Stable metadata on 2026-07-12 resolves Runseal 0.9.0, Flavor 0.3.6, and Sidecar 0.5.0.
- Flavor 0.3.6 and Sidecar 0.5.0 are installed locally; Runseal 0.9.0 was already current.
