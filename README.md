...
Experiment 0152 修正该边界验收中一个真实的跨 helper 隐式状态依赖：首次 v67 全流程在
聚焦测试全部通过后得到 local `(-960,-3648)`，X 落在 cardinal 而非 45-Q9 diagonal
component lattice，证明第二 helper 不能假定 W 一直 held。现在切向阶段在同一
PID/window/thread 原子重申 Shift/W/A，再保持 A 约 500ms 后依次释放。
clean-commit `canonical-prototype-v67` 在 168.853 秒通过，458,350-byte report 中 PID
21676 / window `37227806` / thread 32552 的第二原子前缀仅 0.0022ms，最终
`(-1395,-3738)` 仍证明 31 个 45-Q9 X step 和至少 22 个 tangential-only commit。
1,078 live frames 内零 stall/render block/object effect，Escape exit 0、两值、空
...
Experiment 0159 删除 object-action 的独立 readiness-only baseline：v73 中该进程无原生动作，
3.540 秒后被强制终止，5,787-byte minified report 仅供 Rejected startup/actor 两次等值比较。
v74 改为从 host 实际写盘的同一 schema-2 UTF-8 字节串推导 byte count 与 SHA-256，并直接校验
Activated/Rejected 的路径、signed center/origin、active radius、singleton bounds 与 actor
center。`canonical-prototype-v74` 首轮在 163.148 秒通过，391,418-byte report 比 v73 减少
11,182 bytes（2.78%），旧字段为零；两份文档均为 494 bytes 且独立重算 hash 精确一致。
first/restart 两个 readiness capture 与 19 个 graceful unique PID 全部保留，后者均 exit 0、
拥有 `nativeInput`、两值且 stderr/trailing output 为空；19-pair zero-copy 与 Activated/
Rejected/boundary 行为、103/48/20 tests、0 deny / 5 个既有 warning 均通过。没有替代进程、
产品 Rust、Runtime、GPU/source 或资源清理。

Experiment 0160 删除已完成历史使命的 `invalid-camera-alias` 周期性进程：v74 中它每次单独
耗时 4.520 秒，并以 8,645 minified bytes 的 raw/invariant report 再证明产品已有的 checked
`u8::try_from` 边界。v75 将 exact `0x145` 拒绝与 `0x45` held/pressed/released 全缺失证明
收回 reference-host focused test，中央 guard 反向禁止完整旧 route/helper/validator/report
链。`canonical-prototype-v75` 首轮在 157.466 秒通过，374,253-byte report 比 v74 减少
17,165 bytes（4.39%），18 个 graceful unique PID 均 exit 0、拥有 `nativeInput`、两值且
stderr/trailing output 为空；18-pair zero-copy、first/restart 独立 PID、其余行为门限、
103/48/20 tests 与 0 deny / 5 个既有 warning 均保持。按逢 20 约束，完成 guarded validation
后只清理工作区直接子目录 `target/` 与 `out/`，保留 `.task/`、tracked state 与共享缓存。

Experiment 0161 将 cold workspace 下不可靠的 Activated `250ms` action-to-exit 代理替换为
acceptance-owned 可见帧完成边界：观察器在 recovery input 前绑定 exact PID/window，临时
no-activate topmost 捕获 client pixels，以基线之上至少 64 个固定 Activated-green 像素证明
投影开始，再以连续两次回到 `baseline+16` 内证明 acknowledgement 已结束，随后恢复 z-order
并发送 Escape；10 秒只作为失败上限。`canonical-prototype-v76` 在 150.059 秒通过，冷路径
baseline/peak/completion 为 `1/1237/1`，6 个 Activated samples、2 个 clear samples、37 次
采样于 1.347 秒完成；产品终态仍为 exact 12 Activated、14 suppression、1 commit、0 render
block、两值 stdout。18-process / zero-copy 矩阵、103/48/20 tests、0 deny / 5 个既有 warning
均保持，且没有产品 Rust、Runtime、renderer/GPU/source/synchronization 所有权变化。

Experiment 0162 删除每个 graceful raw launch 重复的 `exitCode/stderr/exitReason/
outputValueCount/trailingOutput` 五个成功别名，以及每个 paired invariant 的
`exactlyTwoValues`。shared runner 仍在返回前精确读取 readiness/completion、限制退出时间、
拒绝非零 status、drain stdout 到 EOF 并拒绝 tail、拒绝 stderr；产品 completion 仍是
outcome/reason 唯一语义 owner。`canonical-prototype-v77` 在 156.947 秒通过，372,526-byte
report 比 v76 减少 3,109 bytes（0.83%），超过 2,022-byte minified 下限。18 个 unique PID
均保留 nativeInput/readiness/completion，reason 为 17 Escape + 1 window-close，旧字段与
paired flag 均为零，zero-copy、Activated 12 帧/16 suppression、其余行为矩阵、103/48/20
tests 与 0 deny / 5 个既有 warning 全部保持。

Experiment 0163 在既有 `runseal :canonical-prototype` 内增加唯一的
`--case=activated-frame` focused 模式：只 build Prototype、cook base 与 startup-traversal
两个中心、运行一个真实 Activated recovery 进程，并复用完整 source oracle、产品终态、
native input、window-composition frame observer 与 zero-copy invariant。断开桌面由
`OpenInputDesktop` 在 build/cook 前失败；RDP desktop DC 的陈旧 D3D12 图像被 exact visible
window 的 `PrintWindow(PW_CLIENTONLY | PW_RENDERFULLCONTENT)` 取代。最终三轮连续 focused
运行分别为 13.956/13.716/13.789 秒；代表轮比 v77 快 11.38 倍，报告 27,314 bytes，保持
12 Activated、13 suppression、1 commit、0 block 与 observer `0/777/0`。完整
`canonical-prototype-v78` 在 160.144 秒通过 18 个 unique PID、17 Escape + 1 window-close、
zero-copy 和全部既有 gate；103/48/20 tests、11 个 acceptance support tests、0 deny /
5 个既有 warning 均通过，没有产品、Runtime、renderer/GPU/source 或 full process-count 变化。

Experiment 0164 将唯一 live skeletal palette 从按 25,601 个候选预留收紧为固定
`2 rigs × 8 clips × 64 phases = 1,024` 个共享姿态槽；128-bone/48-byte affine stride、
候选容量、60 DWORD 根常量与六次固定调度不变。实际 allocation 从 157,292,544 降为
6,291,456 bytes，直接减少 144.006 MiB（96.0%）；同配置进程终点 private bytes 减少
143.539 MiB。两条各 32 样本的 release traversal P50 分别从 0.457728/0.663552 ms
降为 0.408576/0.422912 ms，P95/P99 同样下降；frame/actor GPU、64-publication/
16-lifecycle deep resources、完整 runtime 与总 guard 全部通过。历史 fully-unique
...
`runseal :canonical-prototype --case=activated-frame` is the one-process visible-feedback
debug lane. It requires a connected interactive Windows desktop, builds Prototype, cooks only
the base and startup-traversal centers into an independent collection, and reuses the complete
Activated source-oracle, exact-input, 12-frame acknowledgement, exact visible-window
full-content green rise/clear,
transport, and single-owner invariants. Its generated report is never an input to full acceptance.
Unknown cases fail instead of selecting a fallback.

`runseal :canonical-prototype` is the complete real-process prototype workflow. It runs the
