...
| `.runseal/wrappers/canonical-prototype.ts` | Focused fresh-source prototype input-edge/boundary/gravity/camera-relative Walk/Run/Run-release/re-press/Jump/readmission/midair rejection/held-camera repeat/re-press/invalid-key rejection/opposite-camera cancellation/counter-clockwise wrap/batch-invariant atomic object feedback/action/consumption/presentation/traversal/backpressure, Escape/window-close/focus sessions, restart, failure, and lifecycle entry point. |
...
| `.runseal/support/guard/prototype-session.ts` | Required bounded Escape/window-close/focus/Jump-readmission/midair-rejection/held-camera-repeat/re-press/Run-release/re-press/invalid-key/opposite-camera/counter-clockwise/atomic-object session contract plus forbidden old native timing and transient action surfaces. |
...
| `.runseal/support/prototype/input/mod.ts` | Process-qualified ordered native prototype-window transport, bounded exact-window-thread atomic input/focus-loss batches, suspend/resume/close actions, and schema-3 timing evidence. |
| `.runseal/support/prototype/input/actions.ts` | Named Prototype locomotion, object, capacity-motion, atomic focus-loss, Escape, and window-close native actions. |
| `.runseal/support/prototype/input/sequences.ts` | Composed startup, Jump, Run release/re-press, camera-repeat/re-press, midair, invalid-key, atomic opposite-camera, and counter-clockwise native input sequences. |
...
| `.runseal/support/prototype/sessions/gates.ts` | Bounded Escape/window-close/focus/Jump/Run release/re-press/camera/input/sustained session matrix, baseline comparisons, and exact invariant composition. |
| `.runseal/support/prototype/sessions/mod.ts` | Shared Prototype process readiness/completion framing, staged Run release/re-press input, and post-readiness native-action execution. |
