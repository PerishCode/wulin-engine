...
impl TerrainProjection {
    pub fn for_terrain(config: LoadConfig) -> Result<Self> {
        Self::new(config)
    }

    pub fn for_objects(config: LoadConfig) -> Result<Self> {
        Self::new(config)
    }

    fn new(config: LoadConfig) -> Result<Self> {
    ...
    pub fn region_id(self, active_index: usize) -> Result<u32> {
...
mod tests {
    ...
    fn canonical_aliases_project_identically() {
        ...
        let base_ids = (0..base.active_count())
            .map(|index| base.region_id(index).unwrap())
            .collect::<Vec<_>>();
        for center in [2, 64, 96, 125] {
            ...
            assert_eq!(
                (0..projection.active_count())
                    .map(|index| projection.region_id(index).unwrap())
                    .collect::<Vec<_>>(),
                base_ids
            );
        }
